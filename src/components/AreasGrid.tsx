import {
  ...
  Leaf, MonitorSmartphone, FileText, Landmark
} from "lucide-react";
import type { Area } from "@/lib/content";

const icons = { UserRound, ShieldCheck, UsersRound, User, BriefcaseBusiness, Scale, Leaf, MonitorSmartphone, FileText, Landmark };
  ...
  return (
    <div className={compact ? "areas-grid compact" : "areas-grid"}>
      {areas.map((area) => {
        ...
        return (
          <article className={`area-card ${area.slug === "execucao-penal" ? "area-card-wide" : ""}`} key={area.slug}>
